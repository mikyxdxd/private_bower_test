...
gulp.task('generateABShareModules', function() {

	return gulp.src(['public/src/**/*.ts','!public/src/app/**/*.ts'])
		.pipe(addStream.obj(prepareTemplates()))
		.pipe(sourceMaps.init())
		.pipe(ts({
			noImplicitAny: true,
		...
		.pipe(rename('AbShareModules.min.js'))
		.pipe(uglify())
		.pipe(sourceMaps.write('.'))
		.pipe(gulp.dest('AbShareModules'));
});
...
gulp.task('cssNano', ['sass', 'concatCss'], function() {
	return gulp.src('public/dist/app.css')
		...
		.pipe(gulp.dest('public/dist'))
		.pipe(rename('ABShareModules.min.css'))
		.pipe(gulp.dest('ABShareModules'));
});
...
// Default Task
gulp.task('default', ['lint', 'scripts', 'generateABShareModules', 'sass', 'concatCss', 'cssNano', 'inject', 'serve']);
